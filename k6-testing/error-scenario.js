...
// 1. ANTES do teste, LIGA a simulação de erro
export function setup() {
    const res = http.get(`${PAYMENT_DEBUG_URL}/error/on`);
    if (res.status !== 200) {
        fail('Não foi possível LIGAR a simulação de erro. Abortando teste.');
    }
    console.log('Ambiente preparado: Simulação de ERRO LIGADA.');
}

// 2. O TESTE
export default function () {
    group('Error Scenario Request', function () {
